...
to setup
  ...
  [
    set color green
    setxy -8 6          ;; treci rover se postavlja u gornnju lijevu celiju
...
to go
  if ([xcor] of zelen 0 = 8) and ([ycor] of zelen 0 = 6)
  [ stop ]
  if (length samples1 = 3) or (length samples2 = 3)
  [
    ;; odredjuje se "pobjednik" natjecanja
    ifelse (length samples1 = 3)
    [ ask zelen 0 [win]
      ask smedj 2 [lose] ]
    [ ask smedj 2 [win]
      ask zelen 0 [lose] ]
    stop ]
  ask zelen 0 [ walk1 ]            ;; pokrece se funkcija walk1 za prvi rover
  ask smedj 2 [ walk2 ]            ;; pokrece se funkcija walk2 za drugi rover
  ask zelen 1 [ walk3 ]
  ask smedj 3 [ walk4 ]
  tick
end
...
to walk2  ;; funkcija drugog rovera
  ...
  [
    ...
    [
      set samples2 lput "Orange" samples2
      show samples2
    ]
  ]
  if ([pcolor] of patch-here = red)
  ...
  if ([pcolor] of patch-here = blue)
  [
    if not (member? "Blue" samples2 )
    [
      set samples2 lput "Blue" samples2
      show samples2
    ]
  ]
  ifelse (xcor = -8) and (heading = 270)
...
to walk4  ;; funkcija cetvrtog rovera
  if ([pcolor] of patch-here = orange)
  [
    if not (member? "Orange" samples2 )
    [
