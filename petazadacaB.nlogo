...
to clean
  ...
  wait 0.3
  facexy x-pos y-pos
  ifelse (distancexy x-pos y-pos) > 0.1
  [
    fd 1
  ]
  [
  ...
  if dirt-probability >= 50
                         ;; i ako je veci ili jendak od 95 (5%)
  [
    set templist []
    set randomx random 6
    set randomy random 6
    set templist lput randomx templist
     set templist lput randomy templist
    if [pcolor] of patch randomx randomy != grey[
    set model lput templist model
    ]
     ask patch randomx randomy  [ set pcolor gray ]
  ]
